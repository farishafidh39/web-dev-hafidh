...
class KeranjangController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->has('search')) {
            ...
            $query->where(function ($q) use ($search) {
                foreach (Keranjang::$searchable as $attribute) {
                    $q->orWhere($attribute, 'like', '%' . $search . '%');
                }
            });
        }

        $keranjangs = $query->paginate();
        return view('keranjang.index', [
            'keranjangs' => $keranjangs,
        ])->with('i', (request()->input('page', 1) - 1) * $keranjangs->perPage());
    }
    ...
    public function create()
    {
        ...
        $relatedData = $this->getRelatedData($keranjang);
        $konsumens = Konsumen::all();
        $produks = Produk::all();
        return view('keranjang.create', compact('relatedData', 'konsumens', 'produks'));
    }
    ...
    public function edit($id)
    {
        $keranjang = Keranjang::find($id);
        $relatedData = $this->getRelatedData($keranjang);
        return view('keranjang.edit', array_merge([
            'keranjang' => $keranjang,
        ], $relatedData));
    }
    ...
    public function update(Request $request, Keranjang $keranjang)
    {
        ...
        return redirect()->route('keranjangs.index')->with('success', 'Keranjang updated successfully');
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('keranjangs.index')->with('success', 'Keranjang deleted successfully');
    }
