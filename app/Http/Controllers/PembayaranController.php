...
class PembayaranController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->has('search')) {
            ...
            $query->where(function ($q) use ($search) {
                foreach (Pembayaran::$searchable as $attribute) {
                    $q->orWhere($attribute, 'like', '%' . $search . '%');
                }
            });
        }

        $pembayarans = $query->paginate();
        return view('pembayaran.index', [
            'pembayarans' => $pembayarans,
        ])->with('i', (request()->input('page', 1) - 1) * $pembayarans->perPage());
    }
    ...
    public function create()
    {
        ...
        $relatedData = $this->getRelatedData($pembayaran);
        $pemesanans = Pemesanan::all();
        // dd($pemesanans);
        return view('pembayaran.create', compact('relatedData', 'pemesanans'));
    }
    ...
    {
        request()->validate(Pembayaran::$rules);
    ...
    public function edit($id)
    {
        $pembayaran = Pembayaran::find($id);
        $relatedData = $this->getRelatedData($pembayaran);
        return view('pembayaran.edit', array_merge([
