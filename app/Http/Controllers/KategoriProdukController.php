...
class KategoriProdukController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->has('search')) {
            ...
            $query->where(function ($q) use ($search) {
                foreach (KategoriProduk::$searchable as $attribute) {
                    $q->orWhere($attribute, 'like', '%' . $search . '%');
                }
            });
        }

        $kategoriProduks = $query->paginate();
        return view('kategori-produk.index', [
            'kategoriProduks' => $kategoriProduks,
        ])->with('i', (request()->input('page', 1) - 1) * $kategoriProduks->perPage());
    }
    ...
    public function create()
    {
        ...
        $relatedData = $this->getRelatedData($kategoriProduk);
        return view('kategori-produk.create', $relatedData);
    }
    ...
    {
        request()->validate(KategoriProduk::$rules);
        $kategoriProduk = KategoriProduk::create($request->all());
        return redirect()->route('kategori-produks.index')
            ->with('success', 'KategoriProduk created successfully.');
    }
    ...
    public function show($id)
    {
        ...
        return view('kategori-produk.show', ['kategoriProduk' => $kategoriProduk]);
    }
    ...
    public function edit($id)
    {
        $kategoriProduk = KategoriProduk::find($id);
        $relatedData = $this->getRelatedData($kategoriProduk);
        return view('kategori-produk.edit', array_merge([
    ...
    public function update(Request $request, KategoriProduk $kategoriProduk)
    {
        ...
        return redirect()->route('kategori-produks.index')->with('success', 'KategoriProduk updated successfully');
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('kategori-produks.index')->with('success', 'KategoriProduk deleted successfully');
    }
